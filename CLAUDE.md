...
**On-chain star ratings (built).** Buyers rate a completed order with two 1–5 star scores (quality + delivery speed) on-chain via `rateOrder` (contract: see §4 "On-chain ratings"); no review text. **Storefront:** after a purchase the Checkout "done" step adds a **Confirm & rate** panel — the buyer confirms delivery (`confirmReceipt`, which releases escrow and moves the order to `Completed`) then picks quality + delivery stars and submits (`rateOrder`). The shop hero's HUD shows the seller's **average stars + rating count** (`useSellerRating` → one `sellerRatings(seller)` view read → `summarizeSellerRating`), so any browsing buyer sees a shop's reputation; new shops with no ratings simply hide the badge. **CMS:** the order dashboard shows each Completed order's buyer rating (read-only ★ row, or "buyer hasn't rated yet"). Shared display helpers live in `apps/storefront/src/lib/rating.js` (`summarizeSellerRating`, `starParts`) + a `Stars` component (display + 1–5 picker). No fee/escrow surface touched — ratings are a pure additive layer on the existing lifecycle.
