...
> - **Transport** (`BeeTransport`) is constructed **only** behind the config gate — needs `VITE_BEE_URL` (a full Bee node, not a gateway) + a **messaging** postage batch `VITE_MESSAGING_BATCH_ID` (storefront sends, CMS replies; falls back to the legacy single `VITE_POSTAGE_BATCH_ID`). **Two-batch split** (see `docs/POSTAGE.md`): the durable storage batch (images/metadata, `VITE_STORAGE_BATCH_ID`) is kept separate from the short-lived messaging batch, so the ciphertext self-expires after fulfillment while product content stays alive. The CMS **Storage tab** (`apps/cms/src/lib/postage.js`) creates + tops up + dilutes both from the Bee node's BZZ wallet; pure sizing/health helpers are unit-tested (`postage.test.mjs`).
